...
def main():
    ...
    parser.add_argument(
        "--use_4bit", action="store_true", help="Enable 4-bit quantization"
    )
    ...
    if args.stage in ("all", "data"):
        success &= run_cmd(
            [sys.executable, "scripts/prepare_mock_data.py"], "Prepare Mock Data"
        )
    ...
    if args.stage in ("all", "reward"):
        ...
        cmd = [
            ...
            "config.yaml",
            "--data_path",
            "./data/progress/demo_labels.json",
            "--output_dir",
    ...
    if args.stage in ("all", "grpo"):
        ...
        if not Path(sft_path).exists():
            print(
                f"[ERROR] SFT adapter not found at {sft_path}. Run --stage sft first."
            )
            return
    ...
    if args.stage in ("all", "test"):
        success &= run_cmd(
            [sys.executable, "scripts/end_to_end_test.py"],
